...
// EnsureSignalStrategy stamps the registered strategy instance name when a signal omits it.
func EnsureSignalStrategy(signal *Signal, strategyName string) *Signal {
	if signal == nil {
		return nil
	}
	if strings.TrimSpace(signal.Strategy) == "" && strings.TrimSpace(strategyName) != "" {
