...
func setDbSchema(session *gocql.Session) error {
	...
	// creates diff orders table
	err = session.Query("CREATE TABLE IF NOT EXISTS crypto_trading.diff_orders (book text, d int, r double, t int, a double, v double, oid text, PRIMARY KEY (oid, book, d));").Exec()
	if err != nil {
		return err
	}
	time.Sleep(time.Second)
...
func (cc *CassandraClient) InsertDiffOrderRecord(orders *bitso.WebsocketDiffOrder) error {
	book := orders.Book.String()
	for _, order := range orders.Payload {

		query := cc.session.Query(`INSERT INTO diff_orders (book, d, r, t, a, v, oid) VALUES (?, ?, ?, ?, ?, ?, ?)`, book, order.Timestam, order.Price.Float64(), order.Position, order.Amount.Float64(), order.Value.Float64(), order.OrderID)
