...
func setDbSchema(session *gocql.Session) error {
	...
	// creates trades table v2
	err = session.Query("CREATE TABLE IF NOT EXISTS crypto_trading.trades_v2 (book text, i bigint, a double, r double, v double, morder text, torder text, t int, x text, PRIMARY KEY (i));").Exec()
	if err != nil {
		return err
	}
	time.Sleep(time.Second)
...
func CassandraInitConnection(truncate bool) (*CassandraClient, error) {
...
func (cc *CassandraClient) InsertTradeV2Record(trades *bitso.WebsocketTrade) error {
	book := trades.Book.String()
	for _, trade := range trades.Payload {
		timestampSeconds := int64(trade.CreationTime / 1000) // Convert milliseconds to seconds
		t := time.Unix(timestampSeconds, 0)
		iso8601Time := t.Format(time.RFC3339) // Use RFC3339 layout for ISO8601
		query := cc.session.Query(`INSERT INTO trades_v2 (book, i, a, r, v, morder, torder, t, x) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, book, trade.TID, trade.Amount.Float64(), trade.Price.Float64(), trade.Value.Float64(), trade.MakerOrderID, trade.TakerOrderID, trade.Side, iso8601Time)
